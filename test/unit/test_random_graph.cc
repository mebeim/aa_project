 ...
 * chordal. This is a brute force test of the all elimination orders.
 */
BOOST_AUTO_TEST_CASE(random_chordal_graph_is_chordal) {
	REPEAT(10) {
		Graph g = gen_random_chordal_graph<Graph>(8, 40);
			...
			for (const auto v : ordered) {
				for (const auto a : iter_neighbors(g, v)) {
					for (const auto b : iter_neighbors(g, v)) {
						if (removed.find(a) != removed.end() || removed.find(b) != removed.end())
							continue;
		...
		bad_order:
			ok = false;
		} while (boost::range::next_permutation(ordered)); // 8! = 40320

		BOOST_CHECK_MESSAGE(ok, "no order w/ empty fill-in => graph is not cordal");
	}
}
